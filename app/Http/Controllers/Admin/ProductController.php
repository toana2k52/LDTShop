...
class ProductController extends Controller
{
    ...
    public function index(Request $req)
    {
        ...
        $pros_info = Product_info::paginate(10);
        if ($req->search_pros) {
    ...
    public function post_add(Request $req){
        $this->validate($req,[
            ...
            'connector.required' => 'Cổng kết nối của sản phẩm không được để trống!!!',
            'operating_system.required' => 'Hệ điều hành của sản phẩm không được để trống!!!',
            'design.required' => 'Thiết kế của sản phẩm không được để trống!!!'
        ]);
        $imge = [];
        ...
        if ($prod) {
            ...
            if ($pro_dt) {
                $pro_if = Product_info::create($req->all()); 
                if($pro_if) {
    ...
    public function edit($id, $check, Request $req)
    {
        ...
        $pros_info = Product_info::where('product_id',$id)->get();
        ...
        if ($req->isMethod('post')) {
            ...
            $pros_info = Product_info::where('product_id',$id);
            $imge_ava = $pros->image;
            if($req->hasFile('image_pro')){
                $file = $req->image_pro;
                $file_name_image = date('Y-m-d-h-s-i').$file->getClientOriginalName();
            ...
            if ($pros->update($req->all())) {
                if ($pros_detail->update(['amount'=> $req->amount,'content'=> $req->content,])) {
                    if ($pros_info->update(['cpu'=> $req->cpu,'ram'=> $req->ram,'hard_drive'=> $req->hard_drive,'screen'=> $req->screen,'graphic_card'=> $req->graphic_card,'connector'=> $req->connector,'operating_system'=> $req->operating_system,'design'=> $req->design,'size'=> $req->size])) {
                        if ($check != 0) {
    ...
    return view('admin.product.edit',['pros' => $pros,'cats' =>$cats, 'brands' => $brand, 'pros_detail' => $pros_detail, 'pros_info' => $pros_info, 'check' => $check, 'categories_drop_down' => $categories_drop_down]);
}
...
public function delete($id){
    ...
    $pro_info = Product_info::where('product_id',$id)->delete();
...
public function add_image($id){
    ...
    public function info($id){
        ...
        $pro_if = Product_info::where('product_id',$pro->id)->get();
